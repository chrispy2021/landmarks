...
Inputs:
    ...
    - tracked_histograms : Up-to-date tracked framestem that provides real-time sims which contain landmarks
...
class spatial_filter():
    ...
    '''
    Class member function: filtering
    ...
    def filtering(self, lmk_publisher, correct_hist, tracked_histograms):
        # print("called filtering()")
        
        # voor elke landmar, IF hij min_samples keer gevonden is
        for key in self.lmkObsv:

            # print(len(self.lmkObsv[key]), random.randint(0,4))
            if len(self.lmkObsv[key]) > parameters.min_samples: # 10
                lmk = self.lmkObsv[key]
                gpose = np.array(list(map(operator.itemgetter(2), lmk)))
                # print("gpose: ", gpose)
                gpose = gpose[:,:,0].transpose()
                gpose = np.true_divide(gpose.sum(1),(gpose!=0).sum(1)).reshape(3,1) # hier zit een error
                spatial_filter.pose_check(self,gpose, key)


                # print("in_area: ", self.in_area, "occupied: ", self.occupied, random.randint(0, 5))
                ...
                if self.in_area and not self.occupied:
                    ...
                    for i in range(len(lmk)):
                        ...
                        # lmk_list = convert_landmarklist_to_markerarray(lmk_list) ###MYCODE 
                        lmk_publisher.publish(lmk_list)
                        # print("published", random.randint(0, 5))
                    ...
                    # publish empty landmark
                    # lmk_list = landmark_pub([], key)
                    # lmk_publisher.publish(lmk_list)
            # else:
            #     # publish empty landmark LIST
            #     mt_list = LandmarkList()
            #     lmk_publisher.publish(mt_list)
